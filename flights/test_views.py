...
class FlightViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        passenger = Passenger.objects.create(first="harry", last="potter")
    ...
    def test_cannot_book_nonavailable_seat_flight(self):
        ...
        passenger = Passenger.objects.create(first="hemione", last="granger")
        f = Flight.objects.first()
        f.save()

        c = Client()
        c.post(reverse("book", args=(f.id,)), {"passenger": passenger.id})
